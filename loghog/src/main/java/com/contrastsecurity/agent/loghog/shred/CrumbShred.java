...
public class CrumbShred extends Shred {
    ...
    public CrumbShred() {
        super(new SqlTableBase(CRUMB_TBL_NAME, CRUMB_TBL_CREATE_SQL, CRUMB_TBL_INDEX_SQLS, CRUMB_TBL_COLUMNS),
                new SqlTableBase(MISFITS_TBL_NAME, MISFITS_TBL_CREATE_SQL, MISFITS_TBL_INDEX_SQLS, MISFITS_TBL_COLUMNS),
                ENTRY_SELECTOR, ENTRY_CLASSIFIER, VALUE_EXTRACTOR);
    }

    @Override
    public Object[] transformValues(int line, String entry, String patternId, Map<String, Object> extractedVals) {
        return new Object[] { line, extractedVals.get(LOG_TIMESTAMP_VAR), extractedVals.get(LOG_THREAD_VAR), extractedVals.get(LOG_LOGGER_VAR), extractedVals.get(LOG_LEVEL_VAR), extractedVals.get("message") };
    }
...
create table crumb( 
    ...
    stackframe text) 
""";
    public static final List<String> CRUMB_TBL_INDEX_SQLS = Arrays.asList(
            "create index idx_crumb_url on crumb(url)"
    );
    public static final List<String> CRUMB_TBL_COLUMNS =
            Arrays.asList("line", "type", "req", "resp", "url", "thread", "timestamp", "stackframe");
    ...
    public static final Map<String, List<String>> PATTERN_SIGNATURES = new HashMap<String, List<String>>() {{
    ...
    public static final Map<String, Pattern> VALUE_EXTRACTORS = new HashMap<String, Pattern>() {{
        ...
        put("req_begin", Pattern.compile(DEBUG_PREAMBLE_XTRACT +
                "- CRUMB " + REQ_XTRACT + " " + URL_XTRACT +
                "\\t\\tBEGIN.+"
                + NO_RESP_XTRACT + NO_STACKFRAME_XTRACT + "$"));
        put("hist_resp_begin", Pattern.compile(DEBUG_PREAMBLE_XTRACT +
                "- CRUMB " + RESP_XTRACT + " \\t\\t\\tBEGIN.+" +
                NO_REQ_XTRACT + NO_URL_XTRACT + NO_STACKFRAME_XTRACT + "$"));
        put("resp_begin", Pattern.compile(DEBUG_PREAMBLE_XTRACT +
                "- CRUMB " + RESP_XTRACT + " \\t\\tBEGIN.+" +
                NO_REQ_XTRACT + NO_URL_XTRACT + NO_STACKFRAME_XTRACT + "$"));
        put("req_end", Pattern.compile(DEBUG_PREAMBLE_XTRACT +
                "- CRUMB " + REQ_XTRACT + " " + URL_XTRACT +
                " END & HISTORY:" +
                NO_RESP_XTRACT + NO_STACKFRAME_XTRACT + "$"));
